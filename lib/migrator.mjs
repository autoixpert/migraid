...
export class Migrator {
    ...
    async createMigrationFile({migrationName}) {
        ...
        const template = `import { axLogger } from '@autoixpert/lib/ax-logger/ax-logger';
import {Connection} from "mongoose";

/**
 * What does this migration do? Why?
 */
export default {
    up : async (connection: Connection) => {
        const result = await connection.collection('myCollection').updateMany({
            'path' : {$exists : false}
        }, {
            $set : {
                'path' : ''
        ...
        axLogger.info(\`Updated \${result.modifiedCount} records.\`);
    }
}`;
        try {
