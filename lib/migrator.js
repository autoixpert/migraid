...
class Migrator {
    ...
    connect() {
        ...
        try {
            const configureMongooseModuleExport = require(path.join(process.cwd(), '/src/lib/configure-mongoose.js'));
        ...
        return mongoose.connect(`mongodb://${config.get('migraid.host')}:${config.get('migraid.port')}/${config.get('migraid.database')}`, {useNewUrlParser : true})
            .then((connection) => {
    ...
    async up(connection) {
        ...
        for (const newMigrationFileName of newMigrationFileNames) {
            ...
            const migrationModule = require(path.join(process.env.PWD, config.get('migraid.distDirectory'), newMigrationFileName));
    ...
    async createMigrationFile({migrationName}) {
        ...
        const template = `import {axLogger} from "../lib/ax-logger";
...
export default {
    ...
    async getMigrationFileNames() {
        ...
        for (let fileName of fileNames) {
            ...
            // Only work through .js files, not .ts or .map files
