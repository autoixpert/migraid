...
class Migrator {
    ...
    async up(connection) {
        ...
        for (const newMigrationFileName of newMigrationFileNames) {
            ...
            const migrationModule = require(path.join(process.env.PWD, config.get('migraid.directory'), newMigrationFileName));
    ...
    async createMigrationFile({migrationName}) {
        ...
        const template = `import {axLogger} from "../src/lib/ax-logger";
