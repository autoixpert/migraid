...
class Migrator {
    ...
    connect() {
        ...
        mongoose.connect(`mongodb://${config.get('migraid.host')}:${config.get('migraid.port')}/${config.get('migraid.database')}`, {
                useMongoClient : true,
            })
            .then(() => {
    ...
    async up() {
        ...
        // Return an array of resolved promises. All migrations scripts are executed in order.
        return Promise.all(
            newMigrationFileNames
            // Sort the file names alphabetically, so the oldest files with the lowest ID are added first.
                .sort()
                // The migration file must contain an up function.
                .map(async newMigrationFileName => {
